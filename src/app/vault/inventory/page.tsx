...
export default function InventoryManager() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen bg-[#05070A] text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 pb-24 md:pb-6 custom-scrollbar">
        <div className="max-w-6xl mx-auto space-y-8 sm:space-y-10"> 
          ...
          <section className="space-y-4 sm:space-y-6 pt-6 border-t border-[#1E293B]">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl sm:text-2xl font-black flex items-center gap-2 sm:gap-3"><Edit3 className="text-orange-500" /> Today's Session Ledger</h2>
                <p className="text-gray-400 text-xs sm:text-sm mt-1">Directly edit or delete any session from today.</p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto custom-scrollbar">
                <table className="w-full text-left text-xs sm:text-sm whitespace-nowrap min-w-[800px]">
                  <thead className="text-[10px] uppercase text-gray-500 font-bold bg-[#0B0E14] border-b border-[#1E293B]">
                    <tr>
                      <th className="p-3 sm:p-4">Customer</th><th className="p-3 sm:p-4">System</th><th className="p-3 sm:p-4">Status</th>
                      <th className="p-3 sm:p-4">Time & Dur</th><th className="p-3 sm:p-4">Gaming (₹)</th><th className="p-3 sm:p-4">F&B (₹)</th>
                      <th className="p-3 sm:p-4">Method</th><th className="p-3 sm:p-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#1E293B]">
                    {todaySessions.length === 0 ? (
                      <tr><td colSpan={8} className="p-6 sm:p-8 text-center text-gray-500 italic">No sessions recorded today.</td></tr>
                    ) : (
                      todaySessions.map(session => (
                        <tr key={session.id} className="hover:bg-[#1A2235]/30 transition-colors">
                          <td className="p-3 sm:p-4 font-bold">{session.customer}</td>
                          <td className="p-3 sm:p-4 text-[#00D0FF] font-bold">{session.system}</td>
                          <td className="p-3 sm:p-4"><span className="px-2 py-1 rounded-md text-[9px] sm:text-[10px] font-bold bg-gray-800 text-white">{session.status}</span></td>
                          <td className="p-3 sm:p-4 text-gray-400">{session.entry_time} ({session.duration}h)</td>
                          <td className="p-3 sm:p-4 font-bold">₹{session.total}</td>
                          <td className="p-3 sm:p-4 text-gray-400">₹{session.fnb_total || 0}</td>
                          <td className="p-3 sm:p-4 text-gray-400">{session.method}</td>
                          <td className="p-3 sm:p-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => setEditingSession({...session})} className="p-1.5 sm:p-2 bg-[#1A2235] text-gray-400 hover:text-[#00D0FF] border border-[#2D3748] rounded-lg transition-all"><Edit3 size={14}/></button>
                              <button onClick={() => handleDeleteSession(session.id)} className="p-1.5 sm:p-2 bg-[#1A2235] text-gray-400 hover:text-red-500 border border-[#2D3748] rounded-lg transition-all"><Trash2 size={14}/></button>
                            </div>
                          </td>
                        </tr>
                      ))
      ...
      {editingSession && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-2 sm:p-4">
          <form onSubmit={handleSaveSessionEdit} className="bg-[#121824] p-4 sm:p-6 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto custom-scrollbar border border-orange-500/50 shadow-2xl relative">
            <div className="flex justify-between items-center mb-4 sm:mb-6 border-b border-[#1E293B] pb-3 sm:pb-4">
              <div><h2 className="text-lg sm:text-xl font-black text-white flex items-center gap-2">Master Override <AlertCircle className="text-orange-500" size={16}/></h2></div>
            ...
            <button type="submit" className="w-full bg-orange-500 text-white mt-4 sm:mt-6 p-3.5 sm:p-4 rounded-xl font-black text-sm hover:bg-orange-600 transition-all shadow-[0_0_15px_rgba(249,115,22,0.3)]">Force Save Changes</button>
          </form>
        </div>
      )}
