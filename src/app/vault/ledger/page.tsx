...
import { Monitor, Package, BarChart3, Wallet, TrendingUp, PiggyBank, CreditCard, AlertCircle, Plus, ArrowDownToLine, ArrowUpFromLine, List, Trash2, Landmark, Banknote, Edit3, X } from 'lucide-react';
    ...
    <div className="flex h-screen w-screen bg-[#05070A] text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 custom-scrollbar relative">
        ...
        {!isUnlocked ? (
          <div className="absolute inset-0 flex items-center justify-center bg-[#05070A]">
            <form onSubmit={handleLogin} className="bg-[#121824] p-8 rounded-3xl border border-emerald-500/30 shadow-2xl w-full max-w-sm text-center relative overflow-hidden">
              ...
              <h2 className="text-2xl font-black mb-6 tracking-tight relative z-10">Corporate Ledger</h2>
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4 relative">
             ...
             <div className="absolute -top-3 -right-2 z-20">
                <button onClick={() => { setSyncBank(lifetimeMetrics.liveBank); setSyncCash(lifetimeMetrics.liveCash); setShowSyncModal(true); }} className="bg-[#1A2235] text-gray-400 p-2 rounded-full border border-[#2D3748] hover:text-emerald-400 hover:border-emerald-400 transition-all shadow-xl" title="Sync Balances">
                   <Edit3 size={16}/>
                </button>
             </div>

             <div className="bg-[#121824] p-6 rounded-3xl border border-[#1E293B] shadow-lg flex justify-between items-center">
                <div>
                   <p className="text-emerald-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"><Landmark size={14}/> Live Bank Balance</p>
                   <p className="text-3xl font-black text-white mt-1">₹{formatINR(lifetimeMetrics.liveBank)}</p>
                </div>
             </div>
             <div className="bg-[#121824] p-6 rounded-3xl border border-[#1E293B] shadow-lg flex justify-between items-center">
                <div>
                   <p className="text-[#00D0FF] text-[10px] font-bold uppercase tracking-widest flex items-center gap-2"><Banknote size={14}/> Cash at Home</p>
                   <p className="text-3xl font-black text-white mt-1">₹{formatINR(lifetimeMetrics.liveCash)}</p>
                </div>
             </div>
          </div>

          {/* 10 LAKH ROI TRACKER */}
          <div className="bg-[#121824] p-8 rounded-3xl border border-[#1E293B] shadow-xl relative overflow-hidden">
             <div className="absolute right-0 top-0 w-64 h-64 bg-emerald-500/5 rounded-full blur-3xl -mr-20 -mt-20"></div>
             <div className="flex justify-between items-end mb-4 relative z-10">
                <div>
                  <h2 className="text-2xl font-black text-white flex items-center gap-2"><TrendingUp className="text-emerald-400"/> ROI & Breakeven Tracker</h2>
                  <p className="text-sm text-gray-400 mt-1">Total Net Profit generated vs ₹10 Lakh Initial Investment.</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase text-emerald-400 tracking-widest mb-1">Total Profit Generated</p>
                  <p className="text-4xl font-black tabular-nums">₹{formatINR(lifetimeMetrics.opProfit)}</p>
                </div>
             </div>
             
             <div className="w-full bg-[#0B0E14] rounded-full h-6 border border-[#2D3748] relative z-10 overflow-hidden">
                <div className="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full rounded-full transition-all duration-1000 flex items-center justify-end px-2" style={{ width: `${roiPercentage}%` }}>
                   {roiPercentage > 5 && <span className="text-[10px] font-black text-black">{roiPercentage.toFixed(1)}%</span>}
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form onSubmit={handleAddTransaction} className="lg:col-span-2 bg-[#121824] p-6 rounded-3xl border border-[#1E293B] shadow-lg">
               
               <div className="flex justify-between items-center mb-6 border-b border-[#1E293B] pb-4">
                 <h3 className="text-xl font-black flex items-center gap-2"><CreditCard className="text-[#00D0FF]"/> Record Transaction</h3>
                 
                 <div className="flex bg-[#0B0E14] rounded-lg p-1 border border-[#2D3748]">
                   <button type="button" onClick={() => setEntryType('Expense')} className={`px-4 py-1 text-xs font-bold rounded-md transition-all ${entryType === 'Expense' ? 'bg-red-500/20 text-red-400' : 'text-gray-500 hover:text-white'}`}>Expense / Transfer</button>
                   <button type="button" onClick={() => setEntryType('Income')} className={`px-4 py-1 text-xs font-bold rounded-md transition-all ${entryType === 'Income' ? 'bg-emerald-500/20 text-emerald-400' : 'text-gray-500 hover:text-white'}`}>Manual Income</button>
                 </div>
               </div>
               ...
               <button type="submit" disabled={isProcessing} className={`w-full text-black mt-6 py-4 rounded-xl font-black hover:bg-white transition-all disabled:opacity-50 flex justify-center items-center gap-2 ${entryType === 'Expense' ? 'bg-[#00D0FF] shadow-[0_0_15px_rgba(0,208,255,0.2)]' : 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]'}`}>
                 {isProcessing ? 'Recording...' : <><Plus size={18}/> Record to Ledger</>}
               </button>
            </form>

            {/* PENDING PAYABLES WIDGET */}
            <div className="bg-[#121824] rounded-3xl border border-orange-500/30 shadow-lg flex flex-col">
              <div className="p-6 border-b border-[#1E293B] bg-orange-500/5 rounded-t-3xl">
                <h3 className="text-lg font-black flex items-center gap-2 text-orange-400"><AlertCircle size={18}/> Pending Payables</h3>
                <p className="text-xs text-gray-400 mt-1">Credit Cards & Unpaid Bills.</p>
              </div>
              <div className="flex-1 p-4 overflow-y-auto max-h-[300px] custom-scrollbar">
                {pendingBills.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-gray-500 text-sm italic">No pending bills.</div>
                ) : (
                  <div className="space-y-3">
                    {pendingBills.map(b => (
                      <div key={b.id} className="bg-[#0B0E14] p-3 rounded-xl border border-[#2D3748] flex justify-between items-center group">
                        <div>
                          <p className="text-sm font-bold text-white leading-tight">{b.description}</p>
                          <p className="text-[10px] text-gray-500 mt-1">{b.expense_date} • {b.payment_method}</p>
                        </div>
                        <div className="text-right flex flex-col items-end">
                          <p className="font-black text-orange-400">₹{formatINR(b.amount)}</p>
                          <button onClick={() => markAsPaid(b.id)} disabled={isProcessing} className="mt-2 text-[10px] bg-[#1A2235] px-2 py-1 rounded border border-[#2D3748] hover:border-emerald-500 hover:text-emerald-400 transition-all font-bold disabled:opacity-50">Mark Paid</button>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="bg-[#121824] rounded-3xl border border-[#1E293B] overflow-hidden shadow-xl">
             <div className="p-6 border-b border-[#1E293B] flex justify-between items-center bg-[#0B0E14]">
                <h3 className="text-xl font-black text-white flex items-center gap-3"><PiggyBank className="text-emerald-400"/> Month-over-Month Master PnL</h3>
             </div>
             <div className="overflow-x-auto custom-scrollbar">
               <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-[#1A2235] text-[10px] uppercase text-gray-400 font-black tracking-wider">
                    <tr>
                      <th className="p-4 border-r border-[#2D3748]">Month</th>
                      <th className="p-4 text-[#00D0FF]"><div className="flex items-center gap-1"><ArrowDownToLine size={12}/> Total Income</div></th>
                      <th className="p-4 border-l border-[#2D3748]"><div className="flex items-center gap-1"><ArrowUpFromLine size={12}/> Fixed Cost</div></th>
                      <th className="p-4">F&B Restock</th>
                      <th className="p-4 border-r border-[#2D3748]">Adhoc Exp</th>
                      <th className="p-4 text-emerald-400 border-r border-[#2D3748] bg-emerald-500/5">Net Profit (Before Draws)</th>
                      <th className="p-4 text-orange-400">Loan Repay</th>
                      <th className="p-4 text-orange-400 border-r border-[#2D3748]">Self Drawn</th>
                      <th className="p-4 text-white bg-white/5 font-black">Retained (Bank Bal)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#1E293B]">
                    {monthlyLedger.length === 0 ? (
                      <tr><td colSpan={9} className="p-8 text-center text-gray-500">No financial data found.</td></tr>
                    ) : (
                      monthlyLedger.map((row, i) => (
                        <tr key={i} className="hover:bg-[#1A2235]/30 transition-colors">
                          <td className="p-4 font-bold border-r border-[#2D3748]">{row.month}</td>
                          <td className="p-4 text-[#00D0FF] font-bold text-base">₹{formatINR(row.income)}</td>
                          <td className="p-4 text-red-400 border-l border-[#2D3748]">₹{formatINR(row.fixed)}</td>
                          <td className="p-4 text-red-400">₹{formatINR(row.variable)}</td>
                          <td className="p-4 text-red-400 border-r border-[#2D3748]">₹{formatINR(row.adhoc)}</td>
                          
                          <td className="p-4 font-black text-emerald-400 border-r border-[#2D3748] bg-emerald-500/5 text-lg shadow-inner">
                            ₹{formatINR(row.netProfit)}
                          </td>
                          
                          <td className="p-4 text-orange-400">₹{formatINR(row.loan)}</td>
                          <td className="p-4 text-orange-400 border-r border-[#2D3748]">₹{formatINR(row.self)}</td>
                          <td className="p-4 font-black text-white bg-white/5 text-xl shadow-inner">
          ...
          <div className="bg-[#121824] rounded-3xl border border-[#1E293B] overflow-hidden shadow-xl mt-8">
             <div className="p-6 border-b border-[#1E293B] flex justify-between items-center bg-[#0B0E14]">
                <h3 className="text-xl font-black text-white flex items-center gap-3"><List className="text-[#00D0FF]"/> Complete Expense Log</h3>
                <p className="text-xs text-gray-400">Review your past entries.</p>
             </div>
             <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
               <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-[#1A2235] text-[10px] uppercase text-gray-400 font-black tracking-wider sticky top-0 z-10 shadow-sm">
                    <tr><th className="p-4">Date</th><th className="p-4">Category</th><th className="p-4">Description</th><th className="p-4">Amount</th><th className="p-4">Payment Method</th><th className="p-4">Status</th><th className="p-4 text-right">Action</th></tr>
                  </thead>
                  <tbody className="divide-y divide-[#1E293B]">
                    {rawExpenses.length === 0 ? (
                      <tr><td colSpan={7} className="p-8 text-center text-gray-500">No expenses recorded.</td></tr>
                    ) : (
                      rawExpenses.map((exp, i) => (
                        <tr key={i} className="hover:bg-[#1A2235]/30 transition-colors">
                          <td className="p-4 text-gray-400">{exp.expense_date}</td><td className="p-4 font-bold">{exp.category}</td><td className="p-4 text-white font-bold">{exp.description}</td>
                          <td className="p-4 font-bold text-red-400">₹{formatINR(exp.amount)}</td><td className="p-4 text-gray-400">{exp.payment_method}</td>
                          <td className="p-4"><span className={`px-2 py-1 rounded-md text-[10px] font-bold ${exp.status === 'Paid' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-orange-500/20 text-orange-400'}`}>{exp.status}</span></td>
                          <td className="p-4 text-right"><button onClick={() => handleDeleteExpense(exp.id)} disabled={isProcessing} className="p-2 bg-[#1A2235] text-gray-400 hover:text-red-500 border border-[#2D3748] rounded-lg transition-all"><Trash2 size={14}/></button></td>
                        </tr>
                      ))
                    )}
                  </tbody>
      ...
      {showSyncModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <form onSubmit={handleSyncBalances} className="bg-[#121824] p-6 rounded-3xl w-full max-w-sm border border-[#1E293B] shadow-2xl relative transition-all">
            <div className="flex justify-between items-center mb-6 border-b border-[#1E293B] pb-4">
              <h2 className="text-xl font-black text-white flex items-center gap-2"><Edit3 className="text-emerald-400"/> Hard-Sync Balances</h2>
            ...
            <p className="text-xs text-gray-400 mb-6">Enter the EXACT amount of money you have in reality right now. The system will auto-correct to match this.</p>
            ...
            <button type="submit" disabled={isProcessing} className="w-full bg-emerald-500 text-black py-4 rounded-xl font-black transition-all hover:bg-white disabled:opacity-50 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
